...
function timeSpanToString(startDate, endDate) {
  ...
  const millisec = endDate.getMilliseconds() - startDate.getMilliseconds();
  function zero(item, ...args) {
    let num = item.toString();
    if (num.length < 2) {
      num = `0${num}`;
    }
    if (args[0] === true && num.length < 3) {
      num = `0${num}`;
    }
    return num;
  }
  return `${zero(hours)}:${zero(minutes)}:${zero(seconds)}.${zero(millisec, true)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const corner = Math.abs((hours * 30 + minutes * 0.5) - (minutes * 6));
  const result = Math.min((corner * Math.PI) / 180, Math.PI * 2 - (corner * Math.PI) / 180);
  return result;
}
